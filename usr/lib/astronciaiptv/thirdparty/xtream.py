    ...
    def __init__(self, 
        ...
        if self.cache_path != "":
            ...
            if not osp.isdir(self.cache_path):
                print_with_time("Cache Path is not a directory, using default '~/.cache/astroncia-xtream-cache/'")
                self.cache_path == ""
            ...
            if all_streams != None:
                ...
                for stream_channel in all_streams:
                    if stream_channel['name'] != "":
                        ...
                        print_with_time("Stream has empty name `{}`".format(json.dumps(stream_channel)))
            else:
